...
public class DirectionsJSONParserWayPointsDisplay {
        ...
        try {
            ...
            for (int i = 0; i < jRoutes.length(); i++) {
                ...
                for (int m = 0; m < jLegs.length(); m++) {
                    ...
                    OptimiseMap.distance_time.add(hm_);
...
//                            overview_polyline = ((JSONObject) jRoutes.get(0)).getJSONObject("overview_polyline").getString("points");
                        ...
                        if (jLegs.length() - 1 == m)
                            ...
                            wno = myRouteShipmentList.get((int) places.get((int) waypointOrder.get(m + preposition)).getSpeed() + 1).ItemNo;
//
...
//                        rjRoutes.put(1, ov);
//                        rjRoutes.put(0, ovpolyline);

                        // jrsonObject.getJSONArray("routes").put("legs" , jo);

//                        for (int res = 0; res < waypointOrder.length(); res++) {
//                            if (res == m)
//                                continue;
//
//                            JSONArray rjlegs = ((JSONObject) rjRoutes.get(0)).getJSONArray("legs");
//                            rjlegs.remove(res);
//
////                            if (res == waypointOrder.length() - 1)
//                        dbConnections.InsertPlannedLocationWayPoints(conext, jrsonObject.toString(), m, Integer.parseInt(wno), km, duration, ((JSONObject) jLegs.get(m)).getString("start_address"),
...
//                        tplaces.add(places.get(places.size() - 1));
                ...
                OptimiseMap.places.clear();
                OptimiseMap.places.addAll(tplaces);
