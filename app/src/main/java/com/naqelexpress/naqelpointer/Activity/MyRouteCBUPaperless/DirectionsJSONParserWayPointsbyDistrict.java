...
import static com.naqelexpress.naqelpointer.Activity.MyRouteCBUPaperless.RouteMap_byDistrict.jsonobject_route;
import static com.naqelexpress.naqelpointer.Activity.MyRouteCBUPaperless.RouteMap_byDistrict.precount;
...
public class DirectionsJSONParserWayPointsbyDistrict {
    ...
    public List<List<HashMap<String, String>>> parse(JSONObject jObject, String position, Context conext, int division, int preposition,
                                                     ArrayList<MyRouteShipments> myRouteShipmentList, ArrayList<Location> places, boolean insertdata) {
            ...
            for (int i = 0; i < jRoutes.length(); i++) {
                ...
                for (int m = 0; m < jLegs.length(); m++) {
                    ...
                    if (division == 0) {
                        ...
                        if (waypointOrder == null && ((JSONObject) jRoutes.get(0)).has("waypoint_order")) {
            ...
            if (insertdata) {
                ...
                if (waypointOrder != null && waypointOrder.length() > 0) {
                    for (int i = 0; i < waypointOrder.length() + 1; i++) {
                        if (i == (waypointOrder.length() + 1) - 1) {
                            tplaces.add(places.get(i));
//                        places.remove(0);
                            continue;
                        }
                        int wayposition = (int) waypointOrder.get(i);
                        tplaces.add(places.get(wayposition));
//                    if (waypointOrder.length() - 1 == i)
//                        tplaces.add(places.get(places.size() - 1));
                    }

                    com.naqelexpress.naqelpointer.Activity.routeMap.RouteMap_byDistrict.optimisewaypointslist.addAll(tplaces);

                } else if (waypointOrder != null && waypointOrder.length() == 0) {
                    tplaces.add(places.get(0));
                    RouteMap_byDistrict.optimisewaypointslist.addAll(tplaces);
                }
