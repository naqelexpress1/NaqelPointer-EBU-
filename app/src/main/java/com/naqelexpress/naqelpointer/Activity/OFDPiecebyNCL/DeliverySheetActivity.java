...
import com.naqelexpress.naqelpointer.service.DeliverysheetbyNCL;
    ...
    private void SaveData() {
        ...
        if (IsValid()) {
            ...
            OnCloadingForD onCloadingForD = new OnCloadingForD(GlobalVar.GV().getIntegerFromString
                    ...
                    thirdFragment.PieceBarCodeList.size(), 0,
                    firstFragment.txtTruckID.getText().toString());

            if (dbConnections.InsertOnCloadingForDbyNCL(onCloadingForD, getApplicationContext())) {

                int OnCloadingForDID = dbConnections.getMaxID("OnCloadingForDbyNCL", getApplicationContext());

                for (int i = 0; i < thirdFragment.PieceBarCodeList.size(); i++) { //thirdFragment.PieceBarCodeList
                    String split = thirdFragment.PieceBarCodeList.get(i);

                    OFDPieceLevel onCLoadingForDDetail = new OFDPieceLevel(split,
                            OnCloadingForDID, "");

                    if (!dbConnections.InsertOnCLoadingDetailbyNCLLevel(onCLoadingForDDetail, getApplicationContext())) {
                ...
                if (IsSaved) {
                    ...
                    if (!isMyServiceRunning(DeliverysheetbyNCL.class)) {
                        startService(
                                new Intent(DeliverySheetActivity.this,
                                        DeliverysheetbyNCL.class));
                    }
