    ...
    private boolean IsValid() {
        ...
        if (firstFragment != null) {
            ...
            if (secondFragment.NotDeliveredBarCodeList.size() == 0) {
                GlobalVar.GV().ShowSnackbar(getWindow().getDecorView().getRootView(), "You have to Scan BarCode",
                        GlobalVar.AlertType.Error);
                isValid = false;
            } else if (firstFragment.ShipmentBarCodeList.size() != secondFragment.NotDeliveredBarCodeList.size()) {
                GlobalVar.GV().ShowSnackbar(getWindow().getDecorView().getRootView(), "You have to Scan all pieces",
                        GlobalVar.AlertType.Error);
                isValid = false;
            }
